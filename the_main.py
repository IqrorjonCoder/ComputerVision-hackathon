    ...
    while True:
        ...
        for (top, right, bottom, left), name in zip(face_locations, face_names):
            ...
            if name != 'Unknown':
                ...
                if counter2 != 404:
                    cv2.putText(frame, "Yechildi : 1400 so'm !", (left + 10, bottom - 260), font, 1.0, (40, 180, 5),
                                2)
                    cv2.putText(frame, f"Hisob : {data['karta_hisob']} so'm", (left + 10, bottom - 310), font, 1.0,
                                (40, 180, 5), 2)
                else:
                    cv2.putText(frame, "Pulingiz yetarli emas !", (left - 50, bottom - 260), font, 1.0, (0, 0, 204), 2)
                    cv2.putText(frame, f"Hisob : {data['karta_hisob']} so'm", (left + 10, bottom - 310), font, 1.0,
                                (0, 0, 204),
                                2)
                ...
                cv2.putText(frame, "Bazada yo'qsiz !!!", (left + 10, bottom - 200), font, 1.0, (0, 0, 204), 2)
...
def main():
    ...
    known_face_encodings = [face_recognition.face_encodings(face_recognition.load_image_file(f"./photos/{i}"))[0] for i
                            in into_dir]
